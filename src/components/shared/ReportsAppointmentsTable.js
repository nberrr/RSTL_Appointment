        ...
        <tbody className="divide-y divide-gray-100">
          {appointments.map((appointment, index) => (
            <tr key={appointment.id || index} className="hover:bg-blue-50 hover:border-l-4 hover:border-blue-400 transition-all duration-150 cursor-pointer" onClick={() => onViewDetails(appointment)}>
              ...
              <td className="px-6 py-4 text-sm text-gray-500">{appointment.sample_name || appointment.analysis_requested || ''}</td>
              <td className="px-6 py-4 text-sm text-gray-500">{appointment.sampleDetails?.laboratory || appointment.analysis_requested || ''}</td>
              <td className="px-6 py-4">
