        ...
        <div className="mb-6 border-b border-gray-200">
          <div className="flex space-x-4">
            {['chemical', 'microbiological', 'shelflife'].map((tab) => (
              <button
                key={tab}
                onClick={() => onServiceTabChange(tab)}
                ...
                <label key={service.id} className="flex items-start p-4 bg-white rounded-lg shadow-sm">
                  <div className="flex items-center h-5">
                    <input
                      ...
                      onChange={() => onShelfLifeServiceChange(service.id.replace('-analysis', 'Analysis'))}
          ...
          <div className="space-y-4">
            ...
            {servicesData[appointment.currentServiceTab].categories.map((category, index) => (
              <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
                <button
                  onClick={() => toggleCategory(`${appointment.currentServiceTab}-${index}`)}
