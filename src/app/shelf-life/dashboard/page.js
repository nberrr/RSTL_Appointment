export default function ShelfLifeDashboard() {
  return (
    <div className="p-6">
      <h1 className="text-2xl font-bold mb-6">Shelf Life Laboratory Dashboard</h1>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {/* Studies Overview */}
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-4">Studies Overview</h2>
          <div className="space-y-2">
            <div className="flex justify-between">
              <span>Active Studies</span>
              <span className="font-semibold">0</span>
            </div>
            <div className="flex justify-between">
              <span>Studies Ending Soon</span>
              <span className="font-semibold">0</span>
            </div>
            <div className="flex justify-between">
              <span>Completed Studies</span>
              <span className="font-semibold">0</span>
            </div>
          </div>
        </div>

        {/* Storage Conditions */}
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-4">Storage Conditions</h2>
          <div className="space-y-2">
            <div className="flex justify-between">
              <span>Temperature Alerts</span>
              <span className="font-semibold">0</span>
            </div>
            <div className="flex justify-between">
              <span>Humidity Alerts</span>
              <span className="font-semibold">0</span>
            </div>
            <div className="flex justify-between">
              <span>Storage Units</span>
              <span className="font-semibold">0</span>
            </div>
          </div>
        </div>

        {/* Quick Actions */}
        <div className="bg-white p-6 rounded-lg shadow-md">
          <h2 className="text-xl font-semibold mb-4">Quick Actions</h2>
          <div className="space-y-2">
            <button className="w-full p-2 bg-blue-50 text-blue-700 rounded hover:bg-blue-100">
              New Study
            </button>
            <button className="w-full p-2 bg-green-50 text-green-700 rounded hover:bg-green-100">
              View Studies
            </button>
            <button className="w-full p-2 bg-purple-50 text-purple-700 rounded hover:bg-purple-100">
              Study Reports
            </button>
          </div>
        </div>
      </div>
    </div>
  );
} 